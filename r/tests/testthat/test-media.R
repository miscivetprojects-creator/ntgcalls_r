test_that("media and description objects can be constructed", {
  aud <- ntgcalls::audio_description("test.raw", sample_rate = 48000L, channel_count = 2L)
  ...
  vid <- ntgcalls::video_description("video.raw", width = 1280L, height = 720L, fps = 30L)
  ...
  med <- ntgcalls::media_description(microphone = aud, camera = vid)
...
test_that("data structures and enums validate properly", {
  ...
  dh <- ntgcalls::dh_config(g = 3L, p = charToRaw("prime"), random = charToRaw("rand"))
  expect_s3_class(dh, "ntg_dh_config")
  expect_equal(dh$g, 3L)

  grp <- ntgcalls::ssrc_group("SIM", c(100L, 101L))
  expect_s3_class(grp, "ntg_ssrc_group")
  expect_equal(grp$semantics, "SIM")
  expect_equal(length(grp$ssrcs), 2L)

  map <- ntgcalls::ssrc_mapping(user_id = 12345, ssrc = 999L)
  expect_s3_class(map, "ntg_ssrc_mapping")
  expect_equal(map$ssrc, 999L)

  fd <- ntgcalls::frame_data(absolute_capture_timestamp_ms = 1000, rotation = ntgcalls::VideoRotation$ROTATION_0, width = 1920L, height = 1080L)
  expect_s3_class(fd, "ntg_frame_data")
  expect_equal(fd$width, 1920L)

  f <- ntgcalls::frame(ssrc = 555, data = charToRaw("frame_bytes"), frame_data = fd)
