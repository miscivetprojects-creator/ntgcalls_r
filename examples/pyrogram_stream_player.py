...
async def start_player():
    ...
    logger.info("Loading Telegram dialogs to warm peer cache...")
    ...
    try:
        ...
        join_payload = await ntg.create_call(chat_id)
        ...
        await ntg.connect(chat_id, signaling_data, False)

        logger.info("Configuring live Audio & Video media stream via FFmpeg...")
        temp_dir = tempfile.gettempdir()
        audio_raw_path = os.path.join(temp_dir, f"ntg_audio_{chat_id}.raw")

        logger.info(f"Transcoding audio stream to raw PCM ({audio_raw_path})...")
        ffmpeg_audio_cmd = [
            "ffmpeg", "-y", "-i", stream_info["audio_url"],
            "-f", "s16le", "-ac", "2", "-ar", "48000",
            audio_raw_path
        ]
        proc = subprocess.run(ffmpeg_audio_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if proc.returncode != 0:
            logger.error("FFmpeg audio transcoding failed!")
            return

        audio_desc = ntgcalls.AudioDescription(
            ntgcalls.MediaSource.FILE,
            48000,
            2,
            audio_raw_path,
            False
        ...
        video_desc = None
        enable_video = os.getenv("ENABLE_VIDEO", "0") == "1"
        if enable_video and stream_info.get("video_url"):
            video_raw_path = os.path.join(temp_dir, f"ntg_video_{chat_id}.raw")
            logger.info(f"Transcoding video stream to raw YUV420p ({video_raw_path})...")
            ffmpeg_video_cmd = [
                "ffmpeg", "-y", "-i", stream_info["video_url"],
                "-f", "rawvideo", "-pix_fmt", "yuv420p",
                "-s", "640x360", "-r", "20",
                video_raw_path
            ]
            subprocess.run(ffmpeg_video_cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            if os.path.exists(video_raw_path) and os.path.getsize(video_raw_path) > 0:
        ...
        announcement = (
            ...
            f"⚡ **Powered by:** NTgCalls WebRTC Core"
        )
